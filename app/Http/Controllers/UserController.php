...
public function store(Request $request)
{
    try {
        ...
        if (!empty($validatedData['posicoes'])) {
            $user->posicoes()->sync($validatedData['posicoes']); // sync para evitar duplicatas, mas o attach pode ser usado --ass: Bruno
        }
        ...
        $authController = new AuthUserController(); // login automatico aqui --ass: Bruno
        return $authController->login($request);

    } catch (\Exception $e) { // trataento de erro, melhorar depois --ass: Bruno

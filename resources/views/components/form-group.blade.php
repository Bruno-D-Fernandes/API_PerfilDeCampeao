...
<div class="w-full">
    <label for="{{ $id }}" class="block mb-2 text-md font-medium {{ $theme['label'] }}">
        {{ $label }}      
    </label>
    ...
    <div class="relative">
        <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none text-gray-500">
            ...
            @else
                @if($isPassword)
                    <svg class="w-5 h-5' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-lock-icon lucide-lock"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                @elseif(!$isSelect) 
        ...
        @if($isSelect)
            <select 
                ...
                {{ $attributes->merge([
                    ...
                    $theme['input'] . 
                    (isset($icon) && $icon->isNotEmpty() ? ' ps-10' : '')
                ]) }}
            >
                {{ $slot }}
            </select>
        ...
        @if($isPassword)
            <button 
                ...
                class="absolute inset-y-0 end-0 flex items-center pe-3.5 cursor-pointer text-gray-500 hover:text-gray-700 focus:outline-none"
            >
                ...
                <svg id="{{ $id }}-eye-closed" class="w-5 h-5 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <p id="{{ $id }}-error" class="mt-2 text-sm text-red-600 hidden"></p>
</div>

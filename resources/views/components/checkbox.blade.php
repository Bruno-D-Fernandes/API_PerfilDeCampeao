...
<div class="flex items-center">
    <input 
        ...
        type="checkbox" 
        value="" 
        class="w-[0.83vw] h-[0.83vw] rounded-[0.21vw] border-[0.052vw] {{ $style['borderColor'] }} {{ $style['checkedColor'] }} {{ $style['focusBorder'] }} focus:ring-0 focus:ring-offset-0"
    >
    
    <label for="checkbox" class="select-none ms-[0.42vw] text-[0.83vw] font-medium {{ $style['labelColor'] }}">
        @if($label)
            {{ $label }}
        @elseif(isset($link))
            {{ $link }}

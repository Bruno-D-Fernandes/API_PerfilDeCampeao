@php
    $href = route('clube.listas.show', $list->id);
@endphp
    ...
    <div class="flex-1 flex flex-col justify-between relative p-[1.04vw]">
        <div class="absolute top-[0.63vw] right-[0.63vw] z-10 opacity-0 group-hover:opacity-100 transition-opacity">
            <div class="flex items-center bg-white rounded-[0.42vw] shadow-sm p-[0.21vw] gap-x-[0.21vw]">
                <x-icon-button color="blue" onclick="openModal('event.preventDefault(); event.stopPropagation(); edit-list-{{ $list->id }}')">
                ...
                <x-icon-button color="red" onclick="openModal('event.preventDefault(); event.stopPropagation(); delete-list-{{ $list->id }}')">
